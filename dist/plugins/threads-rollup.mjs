...
${o}
${n}${f}${h}
`}async function p(e={}){let{threadsDir:t="src/threads",output:o="public/worker.js",minify:r=!1,sourcemap:l=!1,target:a="es2020"}=e,n=d.resolve(process.cwd(),t),f=d.resolve(process.cwd(),o),h=await w(n);if(h.length===0){console.warn(`No threads found in ${t}`);return}console.log(`Found ${h.length} thread(s): ${h.map(u=>u.name).join(", ")}`);let c=A(h),i=d.join(n,"__sw-entry__.ts");s.writeFileSync(i,c,"utf-8");try{await T({entryPoints:[i],bundle:!0,outfile:f,format:"iife",platform:"browser",target:Array.isArray(a)?a:[a],minify:r,sourcemap:l,write:!0}),console.log(`\u2713 Built worker.js at ${o}`)}finally{s.existsSync(i)&&s.unlinkSync(i)}}function x(e={}){let{threadsDir:t="src/threads",output:o="dist/worker.js",minify:r,sourcemap:l,target:a}=e;return{name:"servex-thread-threads-rollup",async buildStart(){try{await p({threadsDir:t,output:o,minify:r,sourcemap:l,target:a})}catch(n){this.error(`Failed to build threads: ${n}`)}},async watchChange(n){if(n.includes(t))try{await p({threadsDir:t,output:o,minify:r,sourcemap:l,target:a}),console.log("[servex-thread-threads] Threads rebuilt")}catch(f){console.error(`[servex-thread-threads] Failed to rebuild threads: ${f}`)}}}}var M=x;export{M as default,x as threadsRollupPlugin};
